//
//  LookingPlayerViewController.swift
//  Aggone
//
...
class LookingPlayerViewController: UIViewController {
...
//        hud.textLabel.text = "Please wait"
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        btn_gender.setTitle(getString(key: "gender_man"), for: .normal)
    }
    ...
    @objc func refresh() {
//        hud.show(in: view)
        API.getUserByFilter(sport: selectedSport, gender: gender, type: Constants.PLAYER, name: "", city: city, country: country, age: 0, height: 0, weight: 0, onSuccess: { response in
//            self.hud.dismiss()
            self.users.removeAll()
            for uu in response {
                if uu.available_club == 1 {
                    self.users.insert(uu, at: 0)
                }
            }
...
//            self.hud.dismiss()
    ...
    @IBAction func onClickGender(_ sender: Any) {
        dd_gener = DropDown()
        dd_gener.anchorView = btn_gender
        ...
        dd_gener.selectionAction = { [unowned self] (index: Int, item: String) in
            self.gender = index
            self.btn_gender.setTitle(item, for: .normal)
        }
        dd_gener.show()
    }
...
extension LookingPlayerViewController: UITableViewDelegate, UITableViewDataSource {
...
extension LookingPlayerViewController: LookingPlayerCellDelegate {
}

extension LookingPlayerViewController: UIPickerViewDelegate, UIPickerViewDataSource {
...
extension LookingPlayerViewController: GMSAutocompleteViewControllerDelegate {
