...
class ResultViewController: UIViewController {
    ...
    func setViewController() {
        ...
        hud.show(in: view)
        API.getUserResultSummary(user: user, onSuccess: { response in
            self.hud.dismiss()
            ...
            var sum: [String] = []
            var skills = UIViewController.getSportSkills(sport: self.user.type == Constants.PLAYER ? self.user.sport : Constants.STATS_COACH, apiResult: [])
            for skill in skills {
            ...
            for club in response {
                ...
                skills = UIViewController.getSportSkills(sport: self.user.type == Constants.PLAYER ? self.user.sport : Constants.STATS_COACH, apiResult: club.1)
                var rowData: [String] = []
                var column = 0
                for skill in skills {
...
extension ResultViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 0 {
                cell.label_text.text = getString(key: "club_name")
            } else {
...
extension ResultViewController: IAxisValueFormatter {
